...
public class HttpConnectionHandler extends Thread {
	...
	private void handleRequest() {
		...
		try {
			...
			logger.throwing(HttpConnectionHandler.class.getName(), "handleRequest/collectHeaders", exception);
		}
	...
	private void handleUnsupported(String method) {
		String response = "<html><head><title>Not Implemented</title></head><body>"
						+ "<h3><strong>" + method + "</strong> is not implemented on this server"
						+ "</h3></body></html>";
		String headers = getResponseHeaders(501, response.length(), "text/html");
		try {
			output.writeBytes(headers + response);
			socket.close();
			socketKiller.cancel();
		} catch (IOException exception) {
	...
	private void handleFileNotFound(String path) {
		String response = "<html><head><title>File Not Found</title></head><body>"
						+ "<h3><strong>" + path + "</strong> was not found on this server"
						+ "</h3></body></html>";
		String headers = getResponseHeaders(404, response.length(), "text/html");
		try {
			output.writeBytes(headers + response);
			socket.close();
			socketKiller.cancel();
	...
	private void handleHttpGet(LinkedList<String> headers, String path) {
		...
		if (requestedFile.exists() && requestedFile.canRead()) {
			...
			try {		
				output.writeBytes(getResponseHeaders(200, contentLength, contentType));
	    ...
	    if (urlParts.length > 1) {
	        ...
	        for (String param : query.split("&")) {
	            ...
	            if (key.equalsIgnoreCase("echo")) {
	            	try {
						output.writeBytes(getResponseHeaders(200, value.length(), "text/html"));
	...
	private String getResponseHeaders(int code, long contentLength, String contentType) {
		...
		switch (code) {
			...
			case 501:
				headers.append("501 Not Implemented\r\n");
			case 404:
